 ...
 * - Pair Code implementation inspired by ETHICALMUU & CYBERMUU 
 */

// === 🔮 EXPRESS & SOCKET.IO SERVER INTEGRATION FOR PRO DASHBOARD ===
...
let phoneNumber = "" // Iache wazi ili ifuate namba inayowekwa kwenye Dashboard
...
async function startXeonBotInc() {
    try {
        ...
        const XeonBotInc = makeWASocket({
            ...
            printQRInTerminal: false, // Zima QR kwenye terminal kwa sababu tunatumia dashboard sasa hivi
        ...
        io.once('connection', (socket) => {
            socket.on('get_code', async (num) => {
                try {
                    ...
                    // Omba pairing code kutoka Baileys
                    let code = await XeonBotInc.requestPairingCode(formattedNum)
                    code = code?.match(/.{1,4}/g)?.join("-") || code
                    
                    // Tuma code kurudi kwenye lile card linalocheza la webyako
